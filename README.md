...
- 进度交接（私有）：[agent-progress](https://github.com/defidehathorn389-max/agent-progress)。

私有库需用户授权。没有权限时请求授权，不把其内容复制到公开Skill绕过权限。具体哪一期完成、当前选择、待办、提交版本均只读统一私有进度库。
...
进度管理工具：[agent-progress-skill](https://github.com/defidehathorn389-max/agent-progress-skill)。所有项目状态进入同一个私有agent-progress仓库，不另设视频交接库。
